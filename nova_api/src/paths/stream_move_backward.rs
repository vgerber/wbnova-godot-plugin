...
pub struct StreamMoveBackwardStream {
    socket: WebSocket<MaybeTlsStream<TcpStream>>,
}

impl StreamMoveBackwardStream {
    pub fn from(socket: WebSocket<MaybeTlsStream<TcpStream>>) -> Self {
        StreamMoveBackwardStream { socket: socket }
    }
...
pub struct StreamMoveBackwardPathParameters {
    pub cell: String,
    pub motion: String,
}
pub struct StreamMoveBackwardQueryParameters {
...
pub async fn stream_move_backward(
    host: &str,
    stream_move_backward_path_parameters: &StreamMoveBackwardPathParameters,
    stream_move_backward_query_parameters: &StreamMoveBackwardQueryParameters,
    additional_headers: Option<Vec<(String, String)>>,
) -> Result<StreamMoveBackwardStream, tungstenite::Error> {
    let mut query_parameters: Vec<(&str, String)> = vec![(
        "playback_speed_in_percent",
        stream_move_backward_query_parameters
            .playback_speed_in_percent
            .to_string(),
    )];
    if let Some(ref query_parameter) =
        stream_move_backward_query_parameters.response_coordinate_system
    {
        query_parameters.push(("response_coordinate_system", query_parameter.to_string()));
    }
    if let Some(ref query_parameter) = stream_move_backward_query_parameters.response_rate {
        query_parameters.push(("response_rate", query_parameter.to_string()));
    }
    if let Some(ref query_parameter) =
        stream_move_backward_query_parameters.start_location_on_trajectory
    {
    ...
    let url = format!(
        "{}/cells/{}/motions/{}/executebackward{}",
        host,
        stream_move_backward_path_parameters.cell,
        stream_move_backward_path_parameters.motion,
        query_string
    );
    ...
    Ok(StreamMoveBackwardStream::from(socket))
}
